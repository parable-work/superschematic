...
// TestNestedArraysGuard: sqlgen refuses arrays of arrays (T[][]) until it
// renders them, rather than emitting T[]. The change that teaches sqlgen
...
func TestNestedArraysGuard(t *testing.T) {
	for _, svc := range []string{"fixture-nested-arrays-db"} {
		...
		if err == nil || !strings.HasPrefix(err.Error(), "sqlgen does not support arrays of arrays yet (") {
