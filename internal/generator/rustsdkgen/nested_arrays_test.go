...
// writeNestedArraysSDK writes the Rust SDK crate of
// fixture-nested-arrays-api to sdkDir against the types crate in typesDir.
func writeNestedArraysSDK(t *testing.T, apiOutput *apigen.APIOutput, sdkDir, typesDir string) *SDKOutput {
	t.Helper()
	sdkOutput, err := Generate(apiOutput, naming.Default().RustSDKCrate(nestedArraysService), naming.Default().RustTypesCrate(nestedArraysService), nestedArraysClock)
	if err != nil {
...
// TestWriteSDKGoldenNestedArrays pins every file of the Rust SDK crate for
// fixture-nested-arrays-api: Vec<Vec<T>> arguments and responses and tool
