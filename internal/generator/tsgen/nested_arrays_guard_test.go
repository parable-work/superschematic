...
// TestNestedArraysGuard: tsgen refuses arrays of arrays (T[][]) until it
// renders them, rather than emitting T[]. The change that teaches tsgen
...
func TestNestedArraysGuard(t *testing.T) {
	for _, svc := range []string{"fixture-nested-arrays", "fixture-nested-arrays-db", "fixture-nested-arrays-api"} {
		...
		if err == nil || !strings.HasPrefix(err.Error(), "tsgen does not support arrays of arrays yet (") {
